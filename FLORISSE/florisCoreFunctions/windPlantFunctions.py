...
def avgVelocity(X, Y, Z, Ufield, xTurb, yTurb, zTurb, D, yaw, tilt, model):
...
def computeCpCtPoweraI(turbine, pitch, yaw, tilt, airDensity, output):
    ...
    if turbine.usePitch:
        output.Cp.append(turbine.Cp(output.windSpeed[-1], pitch))
        output.Ct.append(turbine.Ct(output.windSpeed[-1], pitch))
    else:
        output.Cp.append(turbine.Cp(output.windSpeed[-1]))
        output.Ct.append(turbine.Ct(output.windSpeed[-1]))

#    if output.Ct[-1] >= 1.0:
#        output.Ct[-1] = 0.99999999

    Cptmp = (output.Cp[-1]*(np.cos(yaw*np.pi/180.)**turbine.pP) *
             (np.cos(tilt*np.pi/180.)**turbine.pT))

    output.power.append(.5*airDensity *
                        (np.pi*(turbine.rotorDiameter/2)**2)*Cptmp *
                        turbine.gE*output.windSpeed[-1]**3)

    # Compute axial induction factor
    output.aI.append((0.5 / np.cos(yaw * np.pi / 180.)) *
     (1 - np.sqrt(1-output.Ct[-1]*np.cos(yaw*np.pi/180))))
    return output
...
def computeAddedTI(UfieldOrig, xTurb, yTurb, zTurb, Utp,
    ...
    for turbIdx in upwindTurbines:
        ...
        AreaOverlap.append(computeOverlap(Uwake, UfieldOrig))

        # Niayifar and Porte-Agel, "A new analytical model for wind farm power
        # prediction", 2015
        if (xTurb[turbI]-xTurb[turbIdx]) > 0:
            TI_calc.append(model.TIa*(output.aI[upwindTurbines.index(turbIdx)]**model.TIb) *
